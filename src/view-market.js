...
const USER_PREDICTIONS_ABI = [
  ...
  'function claim(uint256 marketId) external',                    // ← mudou de redeemWinnings
  ...
  'function getMarket(uint256) view returns (tuple)', // redundant mas ok
];
...
async function placeBet(option) {
  ...
  try {
    ...
    showLoadingModal('Verificando posição...', '')
    ...
    // ==================== CHECAGEM CORRIGIDA ====================
    let userPos = { exists: false, option: -1, amount: 0n, claimed: false };

    try {
      const pos = await predictionsSigner.getPosition(BigInt(currentMarket.id), state.userAddress);

      userPos = {
        exists: Boolean(pos.exists),
        option: Number(pos.option),
        amount: pos.amount,
        claimed: Boolean(pos.claimed)
      };

      console.log(`🔍 Posição atual:`, {
        exists: userPos.exists,
        option: userPos.option === 0 ? 'A' : userPos.option === 1 ? 'B' : 'Nenhuma',
        amount: formatUnits(userPos.amount, 18)
      });
    } catch (e) {
      console.warn("getPosition falhou:", e.message);
    }

    if (userPos.exists && !userPos.claimed) {
      const ladoAtual = userPos.option === 0 ? 'A' : 'B';
      if (userPos.option !== option) {
        throw new Error(`Você já apostou na opção ${ladoAtual} deste mercado. Não é permitido trocar de lado.`);
      }
    }
    // =========================================================

    // ====================== VERIFICAÇÃO DE MATIC (GÁS) ======================
    showLoadingModal('Verificando taxa...', 'Saldo de MATIC necessário')
    
    try {
      const gasPrice = await state.provider.getGasPrice();
      const gasLimit = 350000n;                    // estimativa segura
      const gasCost = gasPrice * gasLimit;

      const maticBalance = await state.provider.getBalance(state.userAddress);

      if (maticBalance < gasCost) {
        const needed = formatUnits(gasCost - maticBalance, 18);
        throw new Error(`Saldo de MATIC insuficiente. Você precisa de mais ≈ ${Number(needed).toFixed(4)} MATIC para pagar a taxa.`);
      }
    } catch (gasError) {
      console.warn("Não foi possível verificar gás:", gasError.message);
      // Continua mesmo se falhar a verificação (melhor UX)
    }
    // =====================================================================

    // Prossegue com aprovação e compra
    ...
    await syncRealBalanceToFirebase();
...
async function updateUserBalanceInFirebase(betAmount) {
  if (!currentGoogleUser?.uid) return;
  ...
  try {
    ...
    const currentBalance = Number(data.vWalaBalance || 0);

    const newBalance = Math.max(0, currentBalance - Number(betAmount));

    await setDoc(userRef, {
      vWalaBalance: newBalance,
      lastBetAt: serverTimestamp(),
      updatedAt: serverTimestamp()
    }, { merge: true });

    console.log(`✅ Saldo atualizado com sucesso: ${currentBalance} → ${newBalance} vWALA`);
    
  } catch (e) {
    console.error('❌ Erro ao atualizar vWalaBalance no Firebase:', e);
  }
}


// ==================== SINCRONIZAR SALDO REAL DA BLOCKCHAIN ====================
async function syncRealBalanceToFirebase() {
  if (!currentGoogleUser?.uid || !state.userAddress) return;

  try {
    const onChainBalanceStr = await getUserVWalaBalance();
    const numericBalance = parseFloat(onChainBalanceStr);

    const userRef = doc(db, 'users', currentGoogleUser.uid);

    await setDoc(userRef, {
      vWalaBalance: numericBalance,
      lastBalanceSync: serverTimestamp()
    }, { merge: true });

    console.log(`✅ Saldo sincronizado com blockchain: ${numericBalance} vWALA`);
  } catch (e) {
...
async function loadUserHistory() {
  ...
  try {
    ...
    for (const docSnap of snapshot.docs) {
      ...
      let isRedeemed = bet.redeemed === true;
      let won = false;

      // ==================== VERIFICAÇÃO ON-CHAIN (MELHORIA) ====================
      if (!isResolved) {
        try {
          const contract = new Contract(CONTRACT_ADDRESS, USER_PREDICTIONS_ABI, state.provider);
          const marketOnChain = await contract.getMarket(BigInt(marketId));
          
          if (marketOnChain.resolved) {
            isResolved = true;
            won = Number(marketOnChain.winningOption) === Number(bet.option);
            
            // Atualiza o Firestore automaticamente
            await setDoc(docSnap.ref, {
              resolved: true,
              winningOption: Number(marketOnChain.winningOption),
              resolvedAt: Number(marketOnChain.resolvedAt)
            }, { merge: true });
          }
        } catch (e) {
          console.warn(`Não foi possível verificar mercado ${marketId} on-chain`, e);
        }
      } else {
        won = bet.winningOption !== undefined && Number(bet.option) === Number(bet.winningOption);
      }
      // =====================================================================

      let statusHTML = isResolved 
        ? (won ? `<span class="history-status status-resolved">🏆 Ganho</span>` 
               : `<span class="history-status status-resolved">🔴 Perdido</span>`)
      ...
      if (isResolved && !isRedeemed) {
        if (won) {
          ...
          buttonsHTML = `
            <button class="result-btn" onclick="viewMarketResult('${marketId}')">
              🔍 Verificar Resultado
            </button>`;
        }
      }
      ...
      html += `
        <div class="history-item">
          ...
          <div class="history-bet-info">
            ...
            <strong>${bet.option === 0 ? 'A' : 'B'}</strong>
          </div>
          ${buttonsHTML}
        </div>
      `;
...
// ==================== VERIFICAR RESULTADO ====================
window.viewMarketResult = async function(marketId) {
  showLoadingModal('Carregando resultado...', '');

  try {
    const contract = new Contract(CONTRACT_ADDRESS, USER_PREDICTIONS_ABI, state.provider);
    const marketData = await contract.getMarket(BigInt(marketId));

    hideLoadingModal();

    if (!marketData.resolved) {
      showAlert('Mercado ainda não resolvido', 'Aguarde o criador resolver o mercado.', 'error');
      return;
    }

    const winningSide = marketData.winningOption === 0 ? 'A' : 'B';
    const userWon = Number(marketData.winningOption) === Number(/* você pode passar a opção do usuário */);

    showAlert(
      `Resultado do Mercado #${marketId}`,
      `✅ Opção vencedora: <strong>${winningSide}</strong><br><br>Confira no histórico se você ganhou.`
    );

  } catch (error) {
    hideLoadingModal();
    showAlert('Erro ao verificar resultado', error.message, 'error');
  }
};
...
window.redeemWinnings = async function(marketId) {
  ...
  try {
    ...
    const tx = await contract.claim(BigInt(marketId));
    await tx.wait();

    // Atualiza Firestore (marca como resgatado)
    ...
    await syncRealBalanceToFirebase();   // <--- Atualiza com saldo REAL da blockchain
