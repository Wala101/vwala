...
const receipt = await state.provider.waitForTransaction(tx.hash, 1, 120000)

if (!receipt) {
  throw new Error('Transação não confirmada (timeout ou RPC lento)')
}
...
await Promise.all([
  setDoc(doc(db, 'users', currentGoogleUser.uid, 'myMarkets', marketId), marketData),
  setDoc(doc(db, 'markets', marketId), marketData)
])
