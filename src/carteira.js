...
// ====================== FUNÇÃO PARA O NOVO MODAL DE ENVIAR ======================
async function handleSendPolygonWithData(destination, amountStr, pin) {
  if (!currentGoogleUser) {
    openAuthGate();
    return;
  }
  ...
  try {
    const normalizedAmount = normalizeAmountInput(amountStr);
    amountWei = parseEther(normalizedAmount);
    amountNumber = Number(normalizedAmount);
  } catch (error) {
  ...
  const confirmSend = await showConfirmModal(
    'Confirmar envio',
    `<strong>Destino:</strong><br>${escapeHtml(destinationAddress)}<br><br><strong>Valor:</strong><br>${escapeHtml(formatAmount(amountNumber, 'POL'))}`,
    'Enviar',
    'Cancelar'
  );
  ...
  try {
    showLoadingModal('Enviando Polygon', 'Aguarde enquanto sua transação é assinada...');
    ...
    const provider = getRpcProvider('carteira_send_pol_modal', POLYGON_RPC_PRIMARY_URL);
    ...
    if (liveBalanceWei < amountWei + estimatedFeeWei) {
      hideLoadingModal();
      await showMessageModal('Saldo insuficiente', 'Seu saldo não cobre o valor + taxa de rede.');
      return;
    }
    ...
    showLoadingModal('Confirmando na rede', 'Aguarde a confirmação na Polygon...');
    ...
    await showMessageModal(
      'Enviado com sucesso!',
      `<strong>Transferência confirmada.</strong><br><br><strong>Hash:</strong><br>${escapeHtml(formatTxHash(tx.hash))}`
    );
    ...
    await showMessageModal('Erro ao enviar', getSendErrorMessage(error));
  }
}
...
async function handleWalletAction(action) {
  if (action === 'enviar') {
    await openSendModal();     // ← Abre o novo modal
    return
  }
...
<div id="sendModal" class="send-modal hidden">
  <div class="send-modal-content">
    ...
    <div class="send-buttons">
      <button id="sendCancelBtn" class="send-cancel">Cancelar</button>
      <button id="sendConfirmBtn" class="send-confirm">Confirmar</button>
    </div>
  </div>
</div>
  ...
  <!-- RECEBER MODAL COM QR CODE -->
  <div id="receiveModal" class="receive-modal hidden">
    <div class="receive-modal-content">
            
      <div id="qrCodeContainer" class="qr-container"></div>
      
      <button id="copyAddressBtn" class="copy-btn">Copiar Endereço</button>
      <button id="closeReceiveBtn" class="send-cancel">Fechar</button>
    </div>
  </div>
...
// ====================== SELEÇÃO DE ELEMENTOS (sem duplicatas) ======================
...
// ====================== MODAL RECEBER (QR Code) ======================
const receiveModal = document.getElementById('receiveModal');
const copyAddressBtn = document.getElementById('copyAddressBtn');
const closeReceiveBtn = document.getElementById('closeReceiveBtn');

// === GARANTIR QUE NASÇA TOTALMENTE OCULTO ===
if (receiveModal) {
  receiveModal.classList.add('hidden');
  receiveModal.style.display = 'none';           // Força oculto
  receiveModal.style.visibility = 'hidden';      // Extra de segurança
  receiveModal.style.opacity = '0';              // Extra de segurança
}
...
if (closeReceiveBtn) {
  closeReceiveBtn.addEventListener('click', () => {
    if (receiveModal) {
      ...
      receiveModal.style.visibility = 'hidden';
    }
  });
}
...
async function openReceiveModal() {
  ...
  // Gera o QR Code
  try {
    const qrDataUrl = await QRCode.toDataURL(currentWalletAddress, {
      width: 260,
      margin: 2,
      color: { dark: '#00ff9d', light: '#111111' }
    });
    
    qrContainer.innerHTML = `<img src="${qrDataUrl}" alt="QR Code da carteira">`;
  } catch (error) {
...
// ====================== BOTÕES DE PORCENTAGEM (FORA DO CLICK!) ======================
...
quickButtons.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const currentBalance = parseFloat(walletState.polBalance) || 0;
    ...
    if (percent === 100) {
      valueToSet = Math.max(0, currentBalance - parseFloat(POL_GAS_RESERVE));
    } else {
      valueToSet = (currentBalance * percent) / 100;
    }

    // Formata bonito
    amountInput.value = valueToSet.toFixed(6).replace(/\.?0+$/, '');
  });
});
...
if (sendConfirmBtn) {
  sendConfirmBtn.addEventListener('click', async () => {
    const destination = document.getElementById('sendAddress').value.trim();
    const amountStr = document.getElementById('sendAmount').value.trim();
    const pin = document.getElementById('sendPin').value.trim();

    if (!destination || !amountStr || !pin) {
      await showMessageModal('Dados incompletos', 'Preencha todos os campos.');
      return;
    }
    ...
    await handleSendPolygonWithData(destination, amountStr, pin);
  });
}
